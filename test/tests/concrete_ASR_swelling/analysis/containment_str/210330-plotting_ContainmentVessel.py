...
out_file_loc = './test/tests/concrete_ASR_swelling/outputs/containment_str/2021.03.30/'
out_file_name = out_file_loc+'ContainmentVessel3D_v1_out.csv'
...
for i in range(num_row):
    ...
    sliding_x = sliding(out['disp_base_mat_x'][i], out['disp_soil_x'][i], mat_r, soil_r)
    sliding_y = sliding(out['disp_base_mat_y'][i], out['disp_soil_y'][i], mat_r, soil_r)
    sliding_value[i] = tangential_expansion(sliding_x, sliding_y)*1000
...
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
ASR_subfig[i][j].set_ylim(0,0.3)
ASR_subfig[i][j].legend(loc='upper right', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, sliding_value,             c = '0',    linestyle ='solid', linewidth=1.0, label= 'horizontal direction')
ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
