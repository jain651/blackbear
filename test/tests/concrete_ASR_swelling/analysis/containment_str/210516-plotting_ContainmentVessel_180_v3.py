...
def rad_exp_cyl(dx0,dy0,loc0,dx1,dy1,loc1,G):
    return (math.sqrt((loc1[0]+dx1)**2 + (loc1[1]+dy1)**2) - math.sqrt((loc0[0]+dx0)**2 + (loc0[1]+dy0)**2)) / G

def rad_exp_sph(dx0,dy0,dz0,loc0,dx1,dy1,dz1,loc1,G):
    return (math.sqrt((loc1[0]+dx1)**2 + (loc1[1]+dy1)**2 + (loc1[2]+dz1)**2) - math.sqrt((loc0[0]+dx0)**2 + (loc0[1]+dy0)**2 + (loc0[2]+dz0)**2)) / G

def th_exp(dx0,dy0,loc0,dx1,dy1,loc1,G):
    return (math.atan((loc1[1]+dy1)/(loc1[0]+dx1)) - math.atan((loc0[1]+dy0)/(loc0[0]+dx0))) / G

def phi_exp(dx0,dy0,dz0,loc0,dx1,dy1,dz1,loc1,G):
    return (math.atan((loc1[2]+dz1)/math.sqrt((loc1[0]+dx1)**2+(loc1[1]+dy1)**2)) - math.atan((loc0[2]+dz0)/math.sqrt((loc0[0]+dx0)**2+(loc0[1]+dy0)**2))) / G

def sliding_r(dx_top,dy_top,dz_top,loc_top,dx_bot,dy_bot,dz_bot,loc_bot):
    return math.sqrt(((loc_top[0]+dx_top)**2+(loc_top[1]+dy_top)**2)/(loc_top[0]**2+loc_top[1]**2)) - math.sqrt(((loc_bot[0]+dx_bot)**2+(loc_bot[1]+dy_bot)**2)/(loc_bot[0]**2+loc_bot[1]**2))

def sliding_th(dx_top,dy_top,dz_top,loc_top,dx_bot,dy_bot,dz_bot,loc_bot):
    return math.atan((loc_top[1]+dy_top)/(loc_top[0]+dx_top)) - math.atan((loc_bot[1]+dy_bot)/(loc_bot[0]+dx_bot))
...
for i in range(num_row):
    ...
    dome_shade_phi_exp[i] = phi_exp(out['dome_shade_phix0'][i],
                                  ...
                                  dome_shade_phi1,
                                  G_dome_shade_phi)
    base_sun_z_exp[i] = out['base_sun_z'][i]/G_base_sun_z
    cyl_air_sun_z_exp[i] = out['cyl_air_sun_z'][i]/G_cyl_air_sun_z
    cyl_gnd_sun_z_exp[i] = out['cyl_gnd_sun_z'][i]/G_cyl_gnd_sun_z
    base_shade_z_exp[i] = out['base_shade_z'][i]/G_base_shade_z
    cyl_air_shade_z_exp[i] = out['cyl_air_shade_z'][i]/G_cyl_air_shade_z
    cyl_gnd_shade_z_exp[i] = out['cyl_gnd_shade_z'][i]/G_cyl_gnd_shade_z
    ...
    sliding_sun_th[i] = sliding_th(out['base_bot_sun_x'][i],
                                  ...
                                  out['basemat_top_sun_z'][i],
                                  basemat_top_sun)
    sliding_sun_z[i] = out['base_bot_sun_z'][i]- out['basemat_top_sun_z'][i]
    ...
    sliding_shade_th[i] = sliding_th(out['base_bot_shade_x'][i],
                                  ...
                                  out['basemat_top_shade_z'][i],
                                  basemat_top_shade)
    sliding_shade_z[i] = out['base_bot_shade_z'][i]- out['basemat_top_shade_z'][i]
...
ASR_subfig[i][j].plot(t, dome_shade_r_exp,  c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
...
[i, j] = [0, 1]
ASR_subfig[i][j].set_title('Dome-Theta', fontsize=11)
ASR_subfig[i][j].plot(t, dome_sun_th_exp,   c = '0',    linestyle ='solid', linewidth=1.0, label= 'sun')
ASR_subfig[i][j].plot(t, dome_shade_th_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,1)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
[i, j] = [0, 2]
ASR_subfig[i][j].set_title('Dome-Phi', fontsize=11)
ASR_subfig[i][j].plot(t, dome_sun_phi_exp,   c = '0',    linestyle ='solid', linewidth=1.0, label= 'sun')
ASR_subfig[i][j].plot(t, dome_shade_phi_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,1)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, cyl_air_shade_r_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade above ground')
ASR_subfig[i][j].plot(t, cyl_gnd_sun_r_exp,   c = '0',    linestyle ='dashed',  linewidth=1.0, label= 'sun under ground')
ASR_subfig[i][j].plot(t, cyl_gnd_shade_r_exp, c = '0.6',  linestyle ='dashed',  linewidth=1.0, label= 'shade under ground')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
...
[i, j] = [1, 1]
ASR_subfig[i][j].set_title('Cylinder-Theta', fontsize=11)
ASR_subfig[i][j].plot(t, cyl_air_sun_th_exp,   c = '0',    linestyle ='solid', linewidth=1.0, label= 'sun above ground')
ASR_subfig[i][j].plot(t, cyl_air_shade_th_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade above ground')
ASR_subfig[i][j].plot(t, cyl_gnd_sun_th_exp,   c = '0',    linestyle ='dashed',  linewidth=1.0, label= 'sun under ground')
ASR_subfig[i][j].plot(t, cyl_gnd_shade_th_exp, c = '0.6',  linestyle ='dashed',  linewidth=1.0, label= 'shade under ground')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,0.04)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, cyl_air_shade_z_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade above ground')
ASR_subfig[i][j].plot(t, cyl_gnd_sun_z_exp,   c = '0',    linestyle ='dashed',  linewidth=1.0, label= 'sun under ground')
ASR_subfig[i][j].plot(t, cyl_gnd_shade_z_exp, c = '0.6',  linestyle ='dashed',  linewidth=1.0, label= 'shade under ground')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,0.04)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, base_shade_r_exp,  c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,1)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
[i, j] = [2, 1]
ASR_subfig[i][j].set_title('Base-Theta', fontsize=11)
ASR_subfig[i][j].plot(t, base_sun_th_exp,   c = '0',    linestyle ='solid', linewidth=1.0, label= 'sun')
ASR_subfig[i][j].plot(t, base_shade_th_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,1)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, base_shade_z_exp, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("ASR expansion [%]")
# ASR_subfig[i][j].set_ylim(0,1)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, sliding_shade_r, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
...
[i, j] = [3, 1]
ASR_subfig[i][j].set_title('Sliding-Theta', fontsize=11)
ASR_subfig[i][j].plot(t, sliding_sun_th,   c = '0',    linestyle ='solid', linewidth=1.0, label= 'sun')
ASR_subfig[i][j].plot(t, sliding_shade_th, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("Sliding [mm]")
# ASR_subfig[i][j].set_ylim(0,3)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
...
ASR_subfig[i][j].plot(t, sliding_shade_z, c = '0.6',  linestyle ='solid', linewidth=1.0, label= 'shade')
# ASR_subfig[i][j].yaxis.set_minor_locator(MultipleLocator(0.2))
ASR_subfig[i][j].set_ylabel("Sliding [mm]")
# ASR_subfig[i][j].set_ylim(0,3)
ASR_subfig[i][j].legend(loc='best', facecolor='1.0', framealpha=0.5, edgecolor = 'none', ncol=1)
