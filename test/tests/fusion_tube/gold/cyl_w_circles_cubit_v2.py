...
lammps_file = '/Users/amitjain/projects/lammps_1/myfiles/pour_flow_2d_sml_v2_out.168000'
...
cubit.cmd('export mesh "/Users/amitjain/projects/blackbear/test/tests/fusion_tube/gold/fusion_tube.e" dimension 2 overwrite')

## truss boundary
# cubit.cmd('create curve location '+str(x[0])+' '+str(y[0])+' '+str(z)+' location '+str(x[0])+' '+str(y[1])+' '+str(z))
# cubit.cmd('block 3 add curve '+str(cubit.get_last_id('curve')))
# cubit.cmd('nodeset 2 add curve '+str(cubit.get_last_id('curve')))
...
# cubit.cmd('block 3 add surface '+str(cubit.get_last_id('surface')))
#
# cubit.cmd('surface 6 size '+str(t))
# cubit.cmd('mesh surface 6')
# cubit.cmd('refine surface 6 numsplit 2 bias 1.0 depth 1 smooth')
# cubit.cmd('mesh surface all')
#
# cubit.cmd('nodeset 2 add curve 6')
# cubit.cmd('nodeset 10 add curve 12 13 14 15')
# cubit.cmd('nodeset 11 add curve 1')
# cubit.cmd('nodeset 12 add curve 2')
# cubit.cmd('nodeset 13 add curve 3')
# cubit.cmd('nodeset 14 add node 773 734 625 ')

## trial of adding air inside
# cubit.cmd('create surface rectangle width '+str(x[1]-x[0]+2*t)+' height '+str(y[1]-y[0]+2*t)+' zplane ')
# cubit.cmd('move Volume '+str(cubit.get_last_id('volume'))+' x '+str(0.5*(x[1]+x[0]))+' y '+str(0.5*(y[1]+y[0]))+' include_merged ')
# cubit.cmd('create surface rectangle width '+str(x[1]-x[0])+' height '+str(y[1]-y[0])+' zplane ')
# cubit.cmd('move Volume '+str(cubit.get_last_id('volume'))+' x '+str(0.5*(x[1]+x[0]))+' y '+str(0.5*(y[1]+y[0]))+' include_merged ')
# cubit.cmd('block 3 add surface '+str(cubit.get_last_id('surface')))
#
